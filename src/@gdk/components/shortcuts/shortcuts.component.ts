...
@Component({
    selector   : 'gdk-shortcuts',
    templateUrl: './shortcuts.component.html',
    styleUrls  : ['./shortcuts.component.scss']
})
export class GdkShortcutsComponent implements OnInit, AfterViewInit, OnDestroy
{
    shortcutItems: any[];
    navigationItems: any[];
    ...
    ngOnInit(): void
    {
        ...
        this.filteredNavigationItems = this.navigationItems = this._gdkNavigationService.getFlatNavigation(this.navigation);

        if ( this._cookieService.check('GDK2.shortcuts') )
        {
            this.shortcutItems = JSON.parse(this._cookieService.get('GDK2.shortcuts'));
        }
        else
        {
    ...
    toggleShortcut(event, itemToToggle): void
    {
        ...
        for ( let i = 0; i < this.shortcutItems.length; i++ )
        {
            if ( this.shortcutItems[i].url === itemToToggle.url )
            {
                ...
                // Save to the cookies
                this._cookieService.set('GDK2.shortcuts', JSON.stringify(this.shortcutItems));
